var flyOrder = {};
/**
 * 乘机人类型：0代表"成人" 1代表"儿童" 购买保险：01代表"1份" 02代表"不购买"
 */
// 儿童票百分比---航空公司定义
			...
			function() {
				...
				if ($(this).attr("checked") == "checked") {
					...
					if(chFlag==0||chFlag==1||chFlag==2){
						...
						$(obj).find('input[name="passengerName"]').val(vls[0]);// 乘机人姓名
					...
					if (pasCount == 1) {
						$('div[divIndex="myPasengerDiv' + idx + '"]').find(
								'input[name="passengerName"]').val("");// 乘机人姓名
						$('div[divIndex="myPasengerDiv' + idx + '"]').find(
								'input[name="certificationNumber"]').val("");// 乘机人证件号码
						$('div[divIndex="myPasengerDiv' + idx + '"]').find(
...
flyOrder = {
		...
		var str1;
		 ...
		 if((activityPrice!="" && activityPrice!="0") || backActivityPrice!="0"){
			 var html = '<div class="myPassengerDiv" id="myPasengerDiv'
					...
					+ '</dl>'
					+ '<dl class="passTypeDl">'
					+ ' <dt><strong>*</strong>乘机人类型：</dt><dd>'
					+ '<dl class="selects lit2" id="placego_ch'
					+ count
					+ '">'
					+ '<dt class="dt_value"></dt>'
					+ '<dd id="placego">'
					+ '<ul class="clearfix showuls">'
					+ '<li onclick="changepassengerType(this)">成人</li>'
					+ StringPass
					+ '</ul>'
					+ '</dd>'
					+ '<input id="passengerType" type="text" name="passengerType" style="width:44px" onchange="flyOrder.passengerTypeSelectChange(this)" class="showinput passType"   value="成人" />'
					+ '</dl>'
					+ '<span class="show">'
					...
					+'<li>年龄要求：<br/>'
					+ '1. 成人票：法定年龄12周岁（含）以上<br/>'
					+ '2. 儿童票：法定年龄2周岁（含）至12周岁（不含）</li>'
					+ '<li>备注：<br/>'
					+ '1. 乘机人年龄以航班起飞当日计算；<br/>'
					+ '2. 同一订单儿童票购买数量不得超过成人票数量；<br/>'
					+ '3. 儿童乘机须有成人陪伴，无成人陪伴儿童请联系航空公司预订；<br/>'
					+ '4. 如因填写有误导致无法登机，由乘机人自行承担责任</li>'
					+ '</ul>'
					+ '</span>'
					+ '</dd>'
					+ '</dl>'
					+ '<dl>'
					+ '<dt><strong>*</strong>乘机人姓名：</dt>'
					+ ' <dd>'
					...
					+ '<ul>'
					+ '<li class="showtitle">乘客说明</li>'
					+ '<li>1. 乘机人姓名必须与登机所出示证件上的姓名一致</li>'
					+ '<li>2. 如使用中文姓名预订机票且持有护照登机，须确保护照上显示中文姓名</li>'
					+ '<li>3. 持护照登机的乘机人，须严格按照护照上显示的姓名顺序填写（包含中文或英文）</li>'
					+ '<li>4. 如持卡人的姓名中出现生僻字无法输入乘机人姓名，请致电400-77-95568预订机票，并提供生僻字的正确拼音</li>'
					+ '<li>5. 同一订单最多填写9位乘机人，超过此人数订票请拆分填写订单</li>'
					+ '<li>6. 机票预订成功后，姓名不允许变更，如因填写有误导致无法登机，由乘机人自行承担责任</li>'
					+ '</span>'
					+ '</dd>'
					+ '</dl>'
					+ '<dl>'
		            ...
		            + '<li class="tableUl">'
		            +   '<ul ><li style="text-align:center;line-height:40px;font-weight:bold">乘机人类型</li>'
		            +    '<li style="font-weight:bold">填写证件类型</li>'
		            +    '<li style="width:120px;font-weight:bold">填写证件号码</li>'
		            +    '<li style="width:120px;font-weight:bold">登机所持证件</li>'
					...
					+ str1
					+ '</dl> '
					 +' <span class="show"><span class="yd_help"> 保险说明</span>'
					 +'<ul>'
					 +' <li class="showtitle">保险说明</li>'
					 +'<li>【投保公司】： <br>'
					 +'平安养老保险股份有限公司 </li>'
					 +'<li>【保险期间】： <br>'
					 +'保险期间自被保险人持本合同约定航班班机的有效机票到达机场通过安全检查时始，至被保险人抵达目的港走出所乘航班班机的舱门时止 </li>'
					 +'<li>【保险金额】： <br>'
					 +' 1、投保：20元人民币/份<br />'
					 +'2、赔付：60万/份（未成年子女赔付总额不得超过国务院保险监督管理机构规定的限额） </li>'
					 +'<li>【保险有效期】： <br>'
					 +' 如当次航班没有乘坐，保险的有效期是航班起飞之后的1年，过期作废无法退保 </li>'
					 +'<li>【投保验证途径】： <br>'
					 +' 1、中航信的网站验真：登录WWW.TRAVELSKY.COM自助验真<br />'
					 +'2、航联保险经纪有限公司的全国客户服务电话400 810 2688验真<br />'
					 +'3、平安保险公司的全国客服电话95511-1-9验真</li>'
					 +' </ul>'
					 +' </span>'
					+ ' </dd>' + '</dl>' + '</div><script>$(bindSelects);</script>';
			return html;
		 }else{
			 var html = '<div class="myPassengerDiv" id="myPasengerDiv'
				+ count
				...
				+ '位乘机人：</dt>'
				+ '<dd>'
				+ '<a id="deletAbut" href="javascript:void(0)" onclick="flyOrder.delPasenger(\'\',this);" class="blue del">删除</a>'
				+ '</dd>'
				+ '</dl>'
				+ '<dl class="passTypeDl">'
				+ ' <dt><strong>*</strong>乘机人类型：</dt><dd>'
				...
				+ '<dd>'
				+ '<ul>'
				+ '<li onclick="flyOrder.insuranceSelectChange(this);">不需要</li>'
				+ str
				+ '</ul>'
				+ '</dd>'
				+ str1
	...
	delPasenger : function(t, obj) {
		...
		if (pasCount <= 9) {
			...
			$("#addPasengerButton").attr("class", "add");
		}
	},
	/**
	 * 改变客票总价及乘客人数信息
	...
	changeSumPrice : function() {
		...
		var flyPrice = $("#flyPrice").val() * 1;// 单张机票价格
		var backFlyPrice = $("#backFlyPrice").val() * 1;// 单张返程票价格
		var fuelFee = $("#fuelFee").val() * 1;// 燃油费
		var backFuelFee = $("#backFuelFee").val() * 1;// 返程燃油费
		var airPortFee = $("#airPortFee").val() * 1;// 机建费
		var backAirPortFee = $("#backAirPortFee").val() * 1;// 返程机建费
		...
		var childrenAirPortFee = $("#childrenAirPortFee").val() * 1;// 儿童--机建费
		var backChildrenAirPortFee = $("#backChildrenAirPortFee").val() * 1;// 返程儿童--机建费
		...
		// 总票价 -->[(单张票面价+燃油+机建)*成人数量+单人保险*保险数量]+[儿童人数*儿童票价+燃油费+机建费+保险费]
...
//		$("#backSumPrice").html("￥" + (backPersonprice + backChildrenprice));
	...
	addPasengerDiv : function() {
		...
		if (pasCount >= searNum-1) {
			...
			if(searNum==9){
				var str = "<span class='box_red' style='width:335'><h6 class='prompt_s prompt_s1'>此订单最多添加"+searNum+"位乘机人，超过此人数请拆分填写订单</h6></span>";
			}else{
				//var str = "<span class='box_red' style='width:335'><h6 class='prompt_s prompt_s1'>此航班舱位仅剩"+searNum+"个座位，最多添加"+searNum+"位乘机人</h6></span>";
